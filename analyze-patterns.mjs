...
function parseReport(reportPath) {
  ...
  const report = {
    company: null, role: null, archetype: null, legitimacyTier: null,
    finalDecision: null, seniority: null, remote: null, teamSize: null,
    comp: null, domain: null, riskLevel: null, confidence: null,
    nextAction: null, topStrengths: [], scores: {}, gaps: [],
  };
  ...
  if (gapTableMatch) {
    ...
    for (const row of gapRows) {
      ...
      if (cols.length >= 2) {
        ...
        if (!duplicate) {
          report.gaps.push({ description: cols[0], severity: cols[1].toLowerCase(), mitigation: cols[2] || '' });
        }
      }
    }
  }
...
function classifyRemote(raw) {
  ...
  if (/\b(hybrid|on-?site|office|relocat)\b/.test(lower)) return 'hybrid/onsite';
  if (/\b(global|anywhere|worldwide|no restrict|work from anywhere)\b/.test(lower)) return 'global remote';
...
function analyze() {
  ...
  if (entries.length === 0) return { error: 'No applications found in tracker.' };
  ...
  const enriched = entries.map(e => {
    ...
    return {
      ...
      outcome, score, report: reportData,
      remoteBucket: classifyRemote(reportData?.remote || e.notes || ''),
      companySize: classifyCompanySize(reportData?.teamSize || ''),
    };
  });
  ...
  const funnel = {};
  for (const e of enriched) { funnel[e.normalizedStatus] = (funnel[e.normalizedStatus] || 0) + 1; }
  ...
  for (const e of enriched) {
    ...
    if (!e.report?.gaps) continue;
    for (const gap of e.report.gaps) {
      const techs = gap.description.match(/\b(JavaScript|TypeScript|Python|Ruby|Java|Go|Rust|Node\.?js|React|Angular|Vue\.?js|Django|Flask|Rails|PHP|Laravel|Kotlin|Swift|C#|\.NET|MongoDB|MySQL|PostgreSQL|Redis|GraphQL|AWS|GCP|Azure|Docker|Kubernetes)\b/gi);
      if (techs) {
  ...
  if (geoBlocker && geoBlocker.percentage >= 20) {
    recommendations.push({
      ...
      reasoning: `${geoBlocker.frequency} of ${enriched.length} offers are location-restricted.`,
      impact: 'high',
    });
  }
  ...
  return {
    ...
    funnel, scoreComparison, archetypeBreakdown, blockerAnalysis,
    remotePolicy, companySizeBreakdown, scoreThreshold, techStackGaps, recommendations,
  };
}

function printSummary(result) {
  if (result.error) { console.log(`\n${result.error}\n`); return; }
  const { metadata, funnel, scoreComparison, blockerAnalysis, remotePolicy, scoreThreshold, recommendations } = result;
  console.log(`\n${'='.repeat(60)}`);
  console.log(`  Pattern Analysis — ${metadata.analysisDate}`);
  console.log(`  ${metadata.total} applications`);
  console.log(`${'='.repeat(60)}\n`);

  console.log('CONVERSION FUNNEL');
  for (const [status, count] of Object.entries(funnel)) {
    const pct = Math.round((count / metadata.total) * 100);
